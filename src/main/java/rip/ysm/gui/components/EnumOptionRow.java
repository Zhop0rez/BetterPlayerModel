...
public class EnumOptionRow<E extends Enum<E>> extends OptionRow<E> {
    ...
    public void renderOverlay(GuiGraphics g, int mouseX, int mouseY, float partialTick, float scrollDisplay) {
        ...
        for (int i = 0; i < visible; i++) {
            ...
            g.drawString(Minecraft.getInstance().font, enumLabel(values[idx]), listX + 6, itemY + (14 - 8) / 2, -1, true);
        }
    ...
    private Component enumLabel(E value) {
        String key = "gui.better_player_model.config." + option.getTranslationKey() + ".value." + value.name().toLowerCase(java.util.Locale.ROOT);
