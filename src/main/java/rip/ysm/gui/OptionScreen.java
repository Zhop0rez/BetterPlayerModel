...
public abstract class OptionScreen extends Screen {
    ...
    protected void init() {
        ...
        int btnH = 20;
        int gap = 4;
        cancelBtn = new FooterButton(panelRight - btnW, footerY, btnW, btnH, Component.translatable("gui.better_player_model.config.cancel"), this::onCancel);
        saveBtn = new FooterButton(cancelBtn.getX() - btnW - gap, footerY, btnW, btnH, Component.translatable("gui.better_player_model.config.save"), this::onSave);
        applyBtn = new FooterButton(saveBtn.getX() - btnW - gap, footerY, btnW, btnH, Component.translatable("gui.better_player_model.config.apply"), this::onApply);
        undoBtn = new FooterButton(panelLeft, footerY, btnW, btnH, Component.translatable("gui.better_player_model.config.undo"), this::onUndo);
    ...
    public void render(GuiGraphics extractor, int mouseX, int mouseY, float partialTick) {
        ...
        if (!tabButtons.isEmpty()) {
            ...
            g.pose().pushPose();
            if (compactTabs) g.pose().translate(-tabScrollDisplay, 0, 0.0f);
            else g.pose().translate(0, -tabScrollDisplay, 0.0f);
        ...
        g.pose().pushPose();
        g.pose().translate(0, -rowScrollDisplay, 0.0f);
        for (OptionRow<?> row : activeRows) {
