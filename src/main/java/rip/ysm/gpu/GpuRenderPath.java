...
public final class GpuRenderPath {
    ...
    public static boolean tryRender(
        ...
        // needs the same projection and active camera model-view transform.
        ...
        int modelSamplerId = resolveSamplerId(modelTex != null ? null : null);
        ...
        RenderSystem.glBindVertexArray(mesh.vao);
    ...
    private static TextureBinding resolveTextureBinding(AbstractTexture texture) { if (texture == null) return TextureBinding.EMPTY; return new TextureBinding(texture.getId(), 0, 1); }
    ...
    private static int resolveSamplerId(Object sampler) { return 0; }

    private static TextureBinding resolveOverlayTexture(OverlayTexture overlayTexture) { if (overlayTexture == null) return TextureBinding.EMPTY; return new TextureBinding(((com.elfmcys.yesstevemodel.mixin.client.OverlayTextureAccessor) Minecraft.getInstance().gameRenderer.overlayTexture()).getTexture().getId(), 0, 1); }

    private static TextureBinding resolveLightmapTexture(Minecraft mc) { return new TextureBinding(((com.elfmcys.yesstevemodel.mixin.client.LightTextureAccessor) mc.gameRenderer.lightTexture()).ysm$getLightTexture().getId(), 0, 1); }

    private static int resolveClampSamplerId() { return 0; }
