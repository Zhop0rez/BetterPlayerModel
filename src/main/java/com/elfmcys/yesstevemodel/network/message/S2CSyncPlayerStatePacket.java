...
public class S2CSyncPlayerStatePacket {
    private static final int MAX_EFFECT_COUNT = 256;
    private static final int MAX_MOLANG_VAR_COUNT = 4096;
    private static final int MAX_MOLANG_VAR_NAME_LENGTH = 256;
    private static final int MAX_MODEL_SWITCH_ID_LENGTH = 256;
    ...
    public static S2CSyncPlayerStatePacket decode(FriendlyByteBuf buffer) {
        ...
        if ((flags & 4) != 0) {
            ...
            validateCount(effectCount, MAX_EFFECT_COUNT, "effect count");
            if (effectCount == 0) {
        ...
        if ((flags & 2048) != 0) {
            message.modelSwitchId = buffer.readUtf(MAX_MODEL_SWITCH_ID_LENGTH);
        }
        if ((flags & 4096) != 0) {
            message.molangHashId = buffer.readInt();
            int varCount = buffer.readVarInt();
            validateCount(varCount, MAX_MOLANG_VAR_COUNT, "molang variable count");
            if (message.isFullSync()) {
                ...
                for (int i = 0; i < varCount; i++) {
                    roamingVars.put(StringPool.computeIfAbsent(buffer.readUtf(MAX_MOLANG_VAR_NAME_LENGTH)), buffer.readFloat());
                }
            } else if (varCount == 0) {
                message.molangVarData = Int2FloatMaps.EMPTY_MAP;
            } else if (varCount == 1) {
                message.molangVarData = Int2FloatMaps.singleton(StringPool.computeIfAbsent(buffer.readUtf(MAX_MOLANG_VAR_NAME_LENGTH)), buffer.readFloat());
            } else {
                int[] keys = new int[varCount];
                float[] values = new float[varCount];
                for (int i = 0; i < varCount; i++) {
                    keys[i] = StringPool.computeIfAbsent(buffer.readUtf(MAX_MOLANG_VAR_NAME_LENGTH));
