...
public class ModelCommand {
    ...
    public static int setModel(CommandContext<CommandSourceStack> context, boolean ignoreAuth) throws CommandSyntaxException {
        ...
        if (info == null) {
            context.getSource().sendSuccess(() -> Component.translatable("commands.better_player_model.export.not_exist", modelName), true);
        ...
        if (ignoreAuth) {
            targets.forEach(player -> ModelInfoCapability.get(player).ifPresent(cap -> {
                ...
                context.getSource().sendSuccess(() -> Component.translatable("message.better_player_model.model.set.success", modelName, player.getScoreboardName()), true);
            }));
            return Command.SINGLE_SUCCESS;
        }
        targets.forEach(player -> ModelInfoCapability.get(player).ifPresent(cap -> {
            AuthModelsCapability.get(player).ifPresent(authCap -> {
                ...
                context.getSource().sendSuccess(() -> Component.translatable("message.better_player_model.model.set.need_auth", modelName, player.getScoreboardName()), true);
            });
        }));
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int reloadAllPack(CommandContext<CommandSourceStack> context) {
        context.getSource().sendSuccess(() -> Component.translatable("message.better_player_model.model.reload.start"), true);
        ...
        if (!ServerModelManager.loadModels(result -> {
            ...
            if (result.isSuccess()) {
                YSMMessageFormatter.sendServerMessage(context.getSource(), Component.translatable("message.better_player_model.model.reload.complete", Double.valueOf(watch.getTime(TimeUnit.MICROSECONDS) / 1000.0d)), true);
                watch.reset();
                watch.start();
            }
        }, data -> {
            ...
            if (!data.getUuidComponentMap().isEmpty()) {
                ...
                if (PlatformAPI.isServer()) {
                    YSMMessageFormatter.sendServerMessage(context.getSource(), Component.translatable("message.better_player_model.model.sync.complete", Double.valueOf(watch.getTime(TimeUnit.MICROSECONDS) / 1000.0d)), true);
                }
            }
        })) {
            context.getSource().sendFailure(Component.translatable("message.better_player_model.model.reload.in_progress"));
            return Command.SINGLE_SUCCESS;
        }
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int disableModel(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (bool) {
            str = "message.better_player_model.model.disable.true";
        } else {
            str = "message.better_player_model.model.disable.false";
        }
        String str2 = str;
        targets.forEach(player -> ModelInfoCapability.get(player).ifPresent(cap -> {
