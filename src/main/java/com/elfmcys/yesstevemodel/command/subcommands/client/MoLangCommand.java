...
public class MoLangCommand {
    ...
    public static int addWatch(CommandContext<CommandSourceStack> context, MolangWatchRegistry.EvaluationPhase watchRegistry) {
        ...
        try {
            ...
            context.getSource().sendFailure(Component.translatable("message.better_player_model.model.debug_animation.parser_error", e.getMessage()));
            return Command.SINGLE_SUCCESS;
        }
    }
    ...
    private static int executeExperssion(CommandContext<CommandSourceStack> context) {
        ...
        try {
            ...
            if (geoEntity != null) {
                geoEntity.executeExpression(value, true, false, str -> Minecraft.getInstance().player.displayClientMessage(Component.translatable("message.better_player_model.model.debug_animation.result", str), false));
            }
            return Command.SINGLE_SUCCESS;
        } catch (ParseException e) {
            context.getSource().sendFailure(Component.translatable("message.better_player_model.model.debug_animation.parser_error", e.getMessage()));
