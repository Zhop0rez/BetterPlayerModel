...
public class CustomPlayerParrotLayer extends GeoLayerRenderer<CustomPlayerEntity> {
    ...
    private void renderParrot(PoseStack poseStack, MultiBufferSource bufferSource, AnimatedGeoModel model, int packedLightIn, Player player, float limbSwing, float limbSwingAmount, float netHeadYaw, float headPitch, boolean isLeftShoulder) {
        net.minecraft.nbt.CompoundTag tag = isLeftShoulder ? player.getShoulderEntityLeft() : player.getShoulderEntityRight(); Parrot.Variant variant = tag.isEmpty() ? null : net.minecraft.world.entity.animal.Parrot.Variant.byId(tag.getInt("Variant"));
        if (variant == null) {
            return;
        }
        poseStack.pushPose();
        ...
        this.parrotModel.renderOnShoulder(poseStack, bufferSource.getBuffer(RenderType.entityCutout(ParrotRenderer.getVariantTexture(variant))), packedLightIn, OverlayTexture.NO_OVERLAY, limbSwing, limbSwingAmount, netHeadYaw, headPitch, player.tickCount);
        poseStack.popPose();
    }
