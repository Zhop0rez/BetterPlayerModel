...
public final class ModelPreviewRenderer {
    ...
    public static boolean renderQueuedGuiPreview(Object renderState, PoseStack poseStack, Object collector) {
        return false;
    }
    ...
    public static <T extends LivingEntity, TAnimatable extends LivingAnimatable<T>> void renderLivingEntityPreview(GuiGraphics guiGraphics, int left, int top, int right, int bottom, float originX, float originY, float scale, float partialTick, TAnimatable animatable, GeoReplacedEntityRenderer<T, TAnimatable> renderer, boolean disablePreviewRotation, boolean hideEquipment) {
        ...
        float offsetX = toModelOffset(originX, left, right, scale);
        float offsetY = toModelOffset(originY, top, bottom, scale);
        float previewYaw = disablePreviewRotation ? 180.0f : 200.0f;
        renderLivingGuiPreview(poseStack, offsetX, offsetY, partialTick, animatable, renderer, disablePreviewRotation, hideEquipment, previewYaw, false, null);
        poseStack.popPose();
    }
    ...
    private interface GuiPreviewRequest {
        void render(PoseStack poseStack, Object collector);
    }
    ...
    private static final class LivingGuiPreviewRequest implements GuiPreviewRequest {
        ...
        @Override
        public void render(PoseStack poseStack, Object collector) {
    ...
    private static void renderLivingGuiPreview(PoseStack poseStack, float offsetX, float offsetY, float partialTick, LivingAnimatable animatable, GeoReplacedEntityRenderer renderer, boolean disablePreviewRotation, boolean hideEquipment, float previewYaw, boolean extraPlayer, Object collector) {
 ...
 */
    ...
    public static boolean renderCustomLocalPlayerPreview(GuiGraphics guiGraphics, LocalPlayer localPlayer, int left, int top, int right, int bottom, float originX, float originY, float scale, float yaw, float partialTick, boolean extraPlayer) {
        ...
        float offsetX = toModelOffset(originX, left, right, scale);
        float offsetY = toModelOffset(originY, top, bottom, scale);
        renderLivingGuiPreview(poseStack, offsetX, offsetY, partialTick, capability, RendererManager.getPlayerRenderer(), false, false, yaw, extraPlayer, null);
        poseStack.popPose();
        return true;
    }
}
