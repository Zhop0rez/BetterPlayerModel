...
public class AnimationRouletteScreen extends Screen {
    ...
    public void init() {
            ...
            addRenderableWidget(new FlatColorButton(this.centerX - 20, this.centerY - 10, 40, 20, Component.empty(), button -> {
                ...
                public Component getMessage() {
                    if (AnimationLockEvent.isLocked()) {
                        return Component.translatable("gui.better_player_model.roulette.lock_on");
                    }
                    return Component.translatable("gui.better_player_model.roulette.lock_off");
                }
            });
        } else {
            addRenderableWidget(new FlatColorButton(this.centerX - 20, this.centerY - 10, 40, 20, Component.translatable("gui.better_player_model.roulette.stop"), button2 -> {
        ...
        if (this.currentConfigGroup != null) {
            ...
            this.scrollDownButton = new FlatColorButton(this.centerX + 242, this.centerY + 50, 28, 60, Component.literal("в–ј"), button7 -> {
                scrollConfigDown(50);
    ...
    public void render(GuiGraphics extractor, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.pose().translate(0.0f, (float)-this.configScrollOffset, 0.0f);
 ...
 */
    ...
    private void renderKeyBindings(GuiGraphics guiGraphics, int slotIndex, int x, int y) {
        ...
        if (keyMapping.isUnbound()) {
            mutableComponentWithStyle.append(Component.translatable("key.better_player_model.extra_animation.none"));
        } else {
