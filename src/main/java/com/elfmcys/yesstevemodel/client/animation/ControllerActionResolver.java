...
public final class ControllerActionResolver {
    ...
    public static ActionSnapshot snapshot(AnimatableEntity<?> animatable, LivingEntity entity, AnimationEvent<?> event) {
        EntityFrameStateTracker<?> tracker = animatable.getPositionTracker();
        boolean remotePlayer = animatable instanceof PlayerCapability cap && !cap.isLocalPlayerModel();
        float groundSpeed = remotePlayer
                ? getRemotePlayerGroundSpeed((PlayerCapability) animatable, entity, event)
                : getGroundSpeed(entity, tracker, event);
    ...
    public static float getRemotePlayerGroundSpeed(PlayerCapability cap, Entity entity) {
        return getRemotePlayerGroundSpeed(cap, entity, null);
    }

    public static float getRemotePlayerGroundSpeed(PlayerCapability cap, Entity entity, AnimationEvent<?> event) {
        float limbSwingAmount = event == null ? 0.0f : event.getLimbSwingAmount();
        return getRemotePlayerGroundSpeed(cap, entity, limbSwingAmount);
    }

    public static float getRemotePlayerGroundSpeed(PlayerCapability cap, Entity entity, float limbSwingAmount) {
        float strafeInput = cap.getPositionTracker().getStrafeInput();
        float forwardInput = cap.getPositionTracker().getForwardInput();
        float inputSpeed = Mth.sqrt((strafeInput * strafeInput) + (forwardInput * forwardInput));
        if (Float.isFinite(inputSpeed) && inputSpeed > MovementQuery.EPSILON) {
            float actualSpeed = MovementQuery.getRemoteGroundSpeed(entity, cap.getPositionTracker());
            return Math.max(inputSpeed, actualSpeed);
        }

        limbSwingAmount = Math.abs(limbSwingAmount);
        if (Float.isFinite(limbSwingAmount) && limbSwingAmount > MovementQuery.EPSILON) {
