    ...
    private String doExtraTest(LivingEntity entity, InteractionHand hand) {
        ...
        if (anim == UseAnim.SPEAR) {
            if (this.extraTes.contains(anim)) {
                String tridentName = this.extraPre + "trident";
                if (this.innerTest.contains(tridentName)) {
                    return tridentName;
                }
                String spearName = this.extraPre + "spear";
                if (this.innerTest.contains(spearName)) {
                    return spearName;
                }
                return this.extraPre + anim.name().toLowerCase(Locale.US);
            }
            String legacyTridentUse = this.extraPre + "trident";
            if (this.innerTest.contains(legacyTridentUse)) {
                return legacyTridentUse;
            }
            String legacySpearUse = this.extraPre + "spear";
            if (this.innerTest.contains(legacySpearUse)) {
                return legacySpearUse;
            }
