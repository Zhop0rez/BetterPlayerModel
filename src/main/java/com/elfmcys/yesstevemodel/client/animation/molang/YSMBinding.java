...
public class YSMBinding extends ContextBinding {
    ...
    private YSMBinding() {
        ...
        entityVar("boat_left_paddle", ctx -> getBoatPaddleState(ctx, net.minecraft.world.entity.vehicle.Boat.PADDLE_LEFT));
        entityVar("boat_right_paddle", ctx -> getBoatPaddleState(ctx, net.minecraft.world.entity.vehicle.Boat.PADDLE_RIGHT));
        entityVar("boat_left_rowing_time", ctx -> getBoatRowingTime(ctx, net.minecraft.world.entity.vehicle.Boat.PADDLE_LEFT));
        entityVar("boat_right_rowing_time", ctx -> getBoatRowingTime(ctx, net.minecraft.world.entity.vehicle.Boat.PADDLE_RIGHT));
        ...
        livingEntityVar("elytra_rot_x", ctx -> 0.0f);
        livingEntityVar("elytra_rot_y", ctx -> 0.0f);
        livingEntityVar("elytra_rot_z", ctx -> 0.0f);
    ...
    private static boolean isBoatRaft(IContext<Entity> context) {
        Boat boat = getBoat(context.entity());
        return boat != null && boat.getVariant() == Boat.Type.BAMBOO;
    }

    private static float getBoatBodyOffsetY(IContext<Entity> context) {
        Boat boat = getBoat(context.entity());
        return boat != null && boat.getVariant() == Boat.Type.BAMBOO ? BOAT_RAFT_BODY_Y_OFFSET : BOAT_DEFAULT_BODY_Y_OFFSET;
    }
    ...
    private static float getBoatPaddleScale(IContext<Entity> context) {
        Boat boat = getBoat(context.entity());
        return boat != null && boat.getVariant() == Boat.Type.BAMBOO ? BOAT_RAFT_PADDLE_SCALE : BOAT_DEFAULT_PADDLE_SCALE;
    }
