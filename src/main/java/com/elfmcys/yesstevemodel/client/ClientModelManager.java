...
//            System.out.println("Received model hash: " + mHash + ", id: " + modelId + ", unk1: " + isAuth + ", unk2: " + isCustomSkinModel);
            int version = buf.readVarInt(); // ╨╖╨В┬╡╨╣╨ВтА║╨╖┬░┬м╨╣╨ПтАЪ╨╡тА║┬ж╨╢┬м╤Ю╨╢╤Х┬╢╨╕┬зтАЮ╨╢тДЦ┬н╨╣╨МтАЭ╨╖╨Й╨Ж╨╖┬ШтАШ╨╣╤Т╨Б╨╡тА╣┬м╨Ф╨Г╨╣╨М╨Б╨╡┬м┬╢╨╖╥С╤Ь╨╢┬╢тАЬ?5535
...
//            if (true) return;
    ...
    public static void ensureLocalModelsLoaded() {
        ...
        reloadLocalModels(error -> {
            ...
            if (error != null && Minecraft.getInstance().player != null) {
                Minecraft.getInstance().player.displayClientMessage(error, false);
            }
        });
    }
    ...
    public static ModelAssembly getLocalModelContext() {
        ...
        // ╨╖тАШтДв╨┐тДЦ╨В╨╡╨ЕтАЪ╨╢╨И┬░╨╡тА╣┬л╨╡┬з╤Ы╨╢╤Ь╤Ы?        loadDefaultModel();
    ...
    private static String normalizeLocalModelId(String modelId) {
        String normalized = modelId.replace('\\', '/').toLowerCase(Locale.ROOT);
        while (normalized.startsWith("/")) {
            normalized = normalized.substring(1);
        }
        normalized = stripImportExtension(normalized);
        ...
        YSMThreadPool.submit(() -> {
            try {
                ...
                if (!cacheDir.exists() || !cacheDir.isDirectory()) {
                    if (callback != null) {
                        callback.accept(new ExportResult(false, Component.literal("╨╖╨Г╨П╨╢┬░┬н╨╢тДЦ┬н╨╣╤Т╤Ю╨╖тАа╤С╨╡╤Ы╤Щ╨╢┬╡┬а╨╕┬о╤Ц╨╖┬╢╨М╨╖╤ШтАЪ╨╢тАЩ╤Ц╨╖тАЬ╨Б╨╣╨Л╥С╨╢┬а┬л╨╖╥С┬ж╨╖╨ВтА║╨╢╨Б┬╗╨╢╤Ы╤У╨╢┬╡┬а╨╕┬╖╤Ф╨│╨ГтДв╨╢┬╢тАЬ╨╡┬╢тАж╨╖тАЬ╨Б╨╣╨М┬ж? " + folder), "", "", 0));
                    }
                    return;
                }
                ...
                if (callback != null) {
                    callback.accept(new ExportResult(false, Component.literal("╨╣╨Л┬╡╨╡╨ЕтАЩ╨╡тДв╤Ф╨╖╨В┬╡╨╖тАж╨Л╨╡╤Щ┬н╨╢╤Ь┬й╨╡тА║┬й╨▓тАУ╤Ш╨╣╨МтДв╨╢тВм┬а╨╢тАвтАЬ╨╢┬╢тАЬ╨│╤У┬й╨╡тДв╤С╨╣тАУ╤Ч╨╢┬мтАЩ╨╛тАб┬д: " + e.getMessage()), "", "", 0));
                }
            }
        });
    }
