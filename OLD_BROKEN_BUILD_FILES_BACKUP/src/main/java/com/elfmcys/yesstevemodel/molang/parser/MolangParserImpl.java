...
public final class MolangParserImpl implements MolangParser {
        ...
        switch (token.kind()) {
            ...
            case ResourceLocation:
                ...
                if (token.kind() == TokenKind.DOT) {
                    ...
                    if (token.kind() != TokenKind.ResourceLocation) {
    ...
    Expression parseCompound(
        ...
        if (current.kind() == TokenKind.DOT) {
            current = lexer.next();
            if (current.kind() == TokenKind.ResourceLocation) {
                lexer.next();
                return new StructAccessExpression(left, current.value());
            } else {
                throw new ParseException("Expect a ResourceLocation after struct access operator", lexer.cursor());
            }
        }
