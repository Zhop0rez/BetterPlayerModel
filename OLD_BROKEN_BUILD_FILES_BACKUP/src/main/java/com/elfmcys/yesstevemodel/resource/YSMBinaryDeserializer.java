...
//            }
    ...
    private void parseSubEntity(Map<String, RawYsmModel.RawSubEntity> targetMap, String categoryName, int index) {
        ...
        if (format <= 26) {
            ...
            subEntity.ResourceLocation = subModuleName;
        } else {
            subEntity.ResourceLocation = categoryName + "_" + index; // >=26жІ’жњ‰Header Name
        }
        int animationCount = reader.readVarInt();
        for (int i = 0; i < animationCount; ++i) {
            String hash = reader.readString();
        ...
        if (format > 26) {
            ...
            subEntity.ResourceLocation = footerSubModuleName;
        }

        targetMap.put(subEntity.ResourceLocation, subEntity);
    }
    ...
    private RawYsmModel.RawGeometry parseModels() {
        ...
        geo.ResourceLocation = reader.readString();
        ...
        for (RawYsmModel.RawGeometry tempMainModel : tempMainModels) {
            switch (tempMainModel.modelType) {
                ...
                case 3:
                    ...
                    subEntity.ResourceLocation = "minecraft:arrow";
                    model.projectiles.put(subEntity.ResourceLocation, subEntity);
                    break;
                default:
