...
public class YSMFolderDeserializer implements AutoCloseable {
    ...
    private static final long MAX_TOTAL_RESOURCE_BYTES = 512L * 1024L * 1024L;
    ...
    private long totalResourceBytes;
    ...
    private byte[] readResource(String relativePath) {
        ...
        try {
            String normalizedPath = normalizeResourcePath(relativePath);
            if (normalizedPath == null) return null;
            byte[] data = null;

            if (inMemoryFiles == null) {
                Path root = rootPath.toAbsolutePath().normalize();
                Path target = rootPath.resolve(normalizedPath).toAbsolutePath().normalize();
                if (!target.startsWith(root)) {
                    return null;
                }
                if (Files.exists(target) && Files.isRegularFile(target)) {
                    long size = Files.size(target);
                    validateResourceSize(size, normalizedPath);
                    data = Files.readAllBytes(target);
                }
            } else {
                data = inMemoryFiles.get(normalizedPath);
                if (data != null) {
                    validateResourceSize(data.length, normalizedPath);
                }
            }

            if (data != null && !readFilesMd5Map.containsKey(normalizedPath)) {
                reserveResourceBytes(data.length, normalizedPath);
    ...
    private String normalizeResourcePath(String relativePath) {
        ...
        Path normalized = rootPath != null
                ? rootPath.getFileSystem().getPath(normalizedPath).normalize()
                : Path.of(normalizedPath).normalize();
    ...
    private void validateResourceSize(long size, String path) throws IOException {
        if (size < 0 || size > MAX_RESOURCE_BYTES) {
            throw new IOException("Model resource exceeds limit: " + path);
        }
    }

    private void reserveResourceBytes(long size, String path) throws IOException {
        if (totalResourceBytes + size > MAX_TOTAL_RESOURCE_BYTES) {
            throw new IOException("Model resources exceed total limit near: " + path);
        }
    ...
    private void parseSubEntities(JsonElement sectionElem, Map<String, RawYsmModel.RawSubEntity> targetMap, String defaultIdentifier) {
        ...
        for (JsonObject item : items) {
            ...
            sub.ResourceLocation = item.has("__temp_identifier") ? getJsonString(item.get("__temp_identifier")) : (defaultIdentifier + "_" + index);
            ...
            targetMap.put(sub.ResourceLocation, sub);
            index++;
        }
    }
    ...
    private RawYsmModel.RawGeometry parseGeometry(byte[] data, int modelType) {
        ...
        if (geoObj.has("description")) {
            ...
            geo.ResourceLocation = getStr(desc, "ResourceLocation", "");
    ...
    private void parseLegacyFormat() {
        ...
        if (arrowData != null) {
            ...
            arrowSub.ResourceLocation = "arrow";
