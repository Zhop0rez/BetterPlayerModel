...
import com.elfmcys.yesstevemodel.client.ModelManager;
...
public class PlayerModelScreen extends Screen implements IGuiWidget {
    ...
    public PlayerModelScreen() {
        ...
        ModelManager.registerGuiWidget(this);
        ModelManager.ensureLocalModelsLoaded();
        this.modelPackMap = new Object2ReferenceOpenHashMap<>(ModelManager.getModelPackMap());
    }
    ...
    private Map<String, ModelAssembly> buildFilteredModelMap() {
        ...
        if (StringUtils.isBlank(currentPath)) {
            mapNewHashMap.putAll(ModelManager.getModelAssemblyMap());
        }
        ModelManager.getModelAssemblyMap().forEach((str, modelAssembly) -> {
    ...
    private void refreshModelList() {
        ...
        if (this.category == Category.AUTH) {
            AuthModelsCapability.get(localPlayer).ifPresent(cap -> {
                for (Map.Entry<String, ModelAssembly> entry : ModelManager.getModelAssemblyMap().entrySet()) {
    ...
    public void init() {
        ...
        })).setTooltipText("gui.better_player_model.model.info");
        ...
        IconButton importButton = new IconButton(this.guiLeft + 377, this.guiTop + 5, 18, 18, 0, 16, button8 -> {
            ModelManager.reloadLocalModels(error -> {
                if (this.minecraft != null && this.minecraft.player != null) {
        ...
        if (this.selectionMode) {
            ...
            addRenderableWidget(new FlatColorButton(this.guiLeft + 41, this.guiTop + 253, 34, 14, Component.translatable("gui.better_player_model.model_select.move"), button -> runMoveSelection()));
            addRenderableWidget(new FlatColorButton(this.guiLeft + 77, this.guiTop + 253, 44, 14, Component.translatable("gui.better_player_model.model_select.new_category"), button -> runCreateCategory()));
            addRenderableWidget(new FlatColorButton(this.guiLeft + 123, this.guiTop + 253, 44, 14, Component.translatable("gui.better_player_model.model_select.rename_category"), button -> runRenameCategory()));
            addRenderableWidget(new FlatColorButton(this.guiLeft + 5, this.guiTop + 270, 44, 14, Component.translatable("gui.better_player_model.model_select.delete_category"), button -> runDeleteCategory()));
            addRenderableWidget(new FlatColorButton(this.guiLeft + 51, this.guiTop + 270, 38, 14, Component.translatable("gui.better_player_model.model_select.select_all"), button -> selectAllFilteredModels()));
            addRenderableWidget(new FlatColorButton(this.guiLeft + 91, this.guiTop + 270, 38, 14, Component.translatable("gui.better_player_model.model_select.cancel"), button -> {
                this.selectedModelIds.clear();
                this.selectionMode = false;
                init();
            }));
        }
        addRenderableWidget(new FlatColorButton(this.guiLeft + 198, this.guiTop + 215, 52, 14, Component.translatable("gui.better_player_model.pre_page"), button10 -> {
        ...
        addRenderableWidget(new FlatColorButton(this.guiLeft + 308, this.guiTop + 215, 52, 14, Component.translatable("gui.better_player_model.next_page"), button11 -> {
            int currentPage = getCurrentPage();
 ...
 */
    ...
    private void renderSyncStatus(GuiGraphics guiGraphics) {
        ...
        ModelManager.SyncStatus currentState = ModelManager.getSyncStatus();
        switch (currentState.getCurrentState()) {
            case WAITING:
                mutableComponentLiteral = Component.translatable("gui.better_player_model.sync_hint.waiting");
                break;
            case LOADING:
                mutableComponentLiteral = Component.translatable("gui.better_player_model.sync_hint.loading");
                break;
            case PREPARING:
                mutableComponentLiteral = Component.translatable("gui.better_player_model.sync_hint.preparing");
                break;
            case SYNCING:
                if (currentState.getSyncedModels() == 0) {
                    mutableComponentLiteral = Component.translatable("gui.better_player_model.sync_hint.syncing");
                    break;
                } else {
    ...
    public void renderModelPreview(GuiGraphics extractor, int mouseX, int mouseY, float partialTick) {
        ...
        if (localPlayer != null) {
            ...
            PlayerCapability.get(localPlayer).ifPresent(cap -> {
                List<FormattedCharSequence> listSplit = this.font.split(FormattedText.of(ModelManager.getModelContext(cap.getModelId()).map(it -> {
    ...
    private Component getImportTooltip() {
        if (ModelManager.isAllowUpload() && ModelManager.isOysmServer()) {
            return Component.translatable("gui.better_player_model.import.tooltip");
        }
        if (!ModelManager.isOysmServer()) {
            return Component.translatable("gui.better_player_model.import.tooltip.waiting");
        }
    ...
    private void toggleModelSelection(String modelId) {
        ...
        this.selectionStatus = Component.translatable("gui.better_player_model.model_select.count", this.selectedModelIds.size());
        init();
    }
