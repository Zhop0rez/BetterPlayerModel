...
public class ModelSyncStateOverlay implements HudOverlay {
    ...
    public void render(GuiGraphics guiGraphics, Font font, float partialTick, int screenWidth, int screenHeight) {
        ...
        ModelManager.SyncStatus syncStatus = ModelManager.getSyncStatus();

        if (syncStatus.getCurrentState() == ModelManager.SyncState.IDLE) {
            int pendingModelCount = ModelManager.getPendingModelCount();
            if (pendingModelCount > 0) {
                int loadedModelCount = ModelManager.getModelAssemblyMap().size();
        ...
        MutableComponent prefixText = Component.translatable("gui.better_player_model.sync_hint.title");

        switch (syncStatus.getCurrentState()) {
            case WAITING:
                prefixText.append(Component.translatable("gui.better_player_model.sync_hint.waiting").withStyle(ChatFormatting.AQUA));
                break;
            case LOADING:
                prefixText.append(Component.translatable("gui.better_player_model.sync_hint.loading").withStyle(ChatFormatting.GOLD));
                break;
            case PREPARING:
                prefixText.append(Component.translatable("gui.better_player_model.sync_hint.preparing").withStyle(ChatFormatting.LIGHT_PURPLE));
                break;
            case SYNCING:
