    ...
    public static List<Component> buildModelTooltip(ModelAssembly modelAssembly, String locale, String fileName, boolean showAdvancedInfo) {
        ...
        if (extraInfo != null) {
            ...
            if (StringUtils.isNoneBlank(extraInfo.getLicense().getFirst())) {
                tooltipLines.add(Component.translatable("gui.better_player_model.model.license",
                        Component.literal(extraInfo.getLicense().getFirst()).withStyle(ChatFormatting.DARK_GRAY)));
            }
        }

        if (showAdvancedInfo) {
            tooltipLines.add(Component.translatable("gui.better_player_model.model.file", Component.literal(fileName).withStyle(ChatFormatting.DARK_GRAY)));
            tooltipLines.add(Component.translatable("gui.better_player_model.model.hash", Component.literal(modelAssembly.getModelData().getModelHash()).withStyle(ChatFormatting.DARK_GRAY)));

            if (StringUtils.isNoneBlank(modelAssembly.getModelData().getExtra())) {
                tooltipLines.add(Component.translatable("gui.better_player_model.model.extra", Component.literal(modelAssembly.getModelData().getExtra()).withStyle(ChatFormatting.DARK_GRAY)));
            }
            ...
            if (modelAssembly.getModelData().getTimestamp() != 0) {
                ...
                tooltipLines.add(Component.translatable("gui.better_player_model.model.timestamp", Component.literal(formattedDate).withStyle(ChatFormatting.DARK_GRAY)));
            }

            if (StringUtils.isNoneBlank(modelAssembly.getModelData().getRand())) {
                tooltipLines.add(Component.translatable("gui.better_player_model.model.rand", Component.literal(modelAssembly.getModelData().getRand()).withStyle(ChatFormatting.DARK_GRAY)));
            }
        }
        ...
        if (info != null) {
            tooltipLines.add(CommonComponents.space());
            tooltipLines.add(Component.translatable("gui.better_player_model.model.main_model_info", info.getBones(), info.getCubes(), info.getFaces()).withStyle(ChatFormatting.GRAY));
            tooltipLines.add(Component.translatable("gui.better_player_model.model.texture_info", modelAssembly.getAnimationBundle().getTextures().size()).withStyle(ChatFormatting.GRAY));
        }
