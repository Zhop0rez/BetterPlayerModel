...
public final class ModelUploadSession {
    ...
    public static synchronized Component start(String modelId, String fileName, byte[] data) {
        if (instance != null && !instance.isTerminal()) {
            return Component.translatable("gui.better_player_model.import.error.in_progress");
        }
        if (!NetworkHandler.isClientConnected() || !ModelManager.isOysmServer()) {
            return Component.translatable("gui.better_player_model.import.error.waiting_handshake");
        }
        if (!ModelManager.isAllowUpload()) {
            return Component.translatable("gui.better_player_model.import.error.disabled_by_server");
        }
        if (data.length == 0) {
            return Component.translatable("gui.better_player_model.import.error.empty_file");
        }
        if (serverLimitsKnown && data.length > lastMaxTotalBytes) {
            return Component.translatable("gui.better_player_model.import.error.server_limit", formatBytes(lastMaxTotalBytes));
        }
        ImportKind kind = ImportKind.fromFileName(fileName);
        if (kind == ImportKind.UNKNOWN) {
            return Component.translatable("gui.better_player_model.import.error.invalid_extension");
        }
        if (kind == ImportKind.YSM && !isYsmFile(data)) {
            return Component.translatable("gui.better_player_model.import.error.invalid_ysm");
        }
        if (kind == ImportKind.ZIP && !isZipFile(data)) {
            return Component.translatable("gui.better_player_model.import.error.invalid_zip");
        }
    ...
    private static Component getRequestErrorText(byte status) {
        return switch (status) {
            case 1 -> Component.translatable("gui.better_player_model.import.error.model_exists");
            case 2 -> Component.translatable("gui.better_player_model.import.error.file_exceeds_server_limit");
            case 3 -> Component.translatable("gui.better_player_model.import.error.no_permission");
            case 4 -> Component.translatable("gui.better_player_model.import.error.server_busy");
            case 5 -> Component.translatable("gui.better_player_model.import.error.invalid_model_id_or_hash");
            case 6 -> Component.translatable("gui.better_player_model.import.error.disabled_by_server");
            case 7 -> Component.translatable("gui.better_player_model.import.error.unsupported_7z");
            default -> Component.translatable("gui.better_player_model.import.error.status", status);
        };
    }

    private static Component getResponseErrorText(byte status) {
        return switch (status) {
            case 1 -> Component.translatable("gui.better_player_model.import.error.hash_mismatch");
            case 2 -> Component.translatable("gui.better_player_model.import.error.server_parse_failed");
            case 3 -> Component.translatable("gui.better_player_model.import.error.server_storage");
            case 4 -> Component.translatable("gui.better_player_model.import.error.session_expired");
            case 5 -> Component.translatable("gui.better_player_model.import.error.incomplete_upload");
            case 6 -> Component.translatable("gui.better_player_model.import.error.server_rejected_write");
            case 8 -> Component.translatable("gui.better_player_model.import.error.scan_not_visible");
            default -> Component.translatable("gui.better_player_model.import.error.status", status);
        };
    }
