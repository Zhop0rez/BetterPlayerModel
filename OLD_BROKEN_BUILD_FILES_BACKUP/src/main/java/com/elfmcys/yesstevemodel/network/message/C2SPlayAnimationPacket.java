...
public class C2SPlayAnimationPacket {
    private static final int MAX_CATEGORY_LENGTH = 256;
    ...
    public static C2SPlayAnimationPacket decode(FriendlyByteBuf buf) {
        return new C2SPlayAnimationPacket(buf.readVarInt(), buf.readUtf(MAX_CATEGORY_LENGTH), buf.readVarInt());
    }
    ...
    private static void handleCapability(C2SPlayAnimationPacket message, ServerPlayer sender) {
        ...
        if (message.entityId != -1) {
            ...
            if (TouhouMaidCompat.isMaidEntity(entity) && sender.distanceToSqr(entity) <= MAX_REMOTE_ENTITY_DISTANCE_SQR) {
