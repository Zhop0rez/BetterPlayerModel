...
public class S2CVersionCheckPacket {
    ...
    public static S2CVersionCheckPacket decode(FriendlyByteBuf buf) {
        String version = buf.readUtf(MAX_VERSION_LENGTH);
        ...
        if (buf.readableBytes() == 1) {
            ...
            try {
                String brand = buf.readUtf(MAX_BRAND_LENGTH);
    ...
    public static void handle(S2CVersionCheckPacket message, PacketContext ctx) {
        ctx.enqueueWork(() -> {
            ModelManager.setOysmServer(message.oysmServer);
            ModelManager.setAllowUpload(message.allowUpload);
        });
        if (NetworkHandler.setChannelVersion(ctx.getConnection(), message.version)) {
            ctx.enqueueWork(ModelManager::onSyncConnected);
        }
