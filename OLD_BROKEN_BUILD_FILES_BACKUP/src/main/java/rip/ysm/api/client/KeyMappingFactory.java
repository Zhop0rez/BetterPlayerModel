...
public final class KeyMappingFactory {
    ...
    private static KeyMapping.Category getOrCreateCategory(String categoryKey) {
        return CATEGORY_CACHE.computeIfAbsent(categoryKey, k -> {
            // MC 26.x: Category uses ResourceLocation; label() generates "key.category.<ns>.<path>"
            return KeyMapping.Category.register(ResourceLocation.fromNamespaceAndPath("better_player_model", "keys"));
        });
    }
