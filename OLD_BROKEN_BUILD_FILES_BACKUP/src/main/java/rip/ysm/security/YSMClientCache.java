...
public class YSMClientCache {
    private static final long MAX_CACHE_FILE_BYTES = 512L * 1024L * 1024L;
    ...
    public static boolean verifyFileContent(File cacheFile, long hash1, long hash2, byte[] rtKey) {
        if (cacheFile == null || !cacheFile.exists() || cacheFile.length() <= 8 || cacheFile.length() > MAX_CACHE_FILE_BYTES) {
            return false;
        }
